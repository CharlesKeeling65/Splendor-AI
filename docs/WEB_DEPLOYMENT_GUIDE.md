...
## 9. 场景 D：只读辅助面板（play-advisor，人在自己座位打牌）

> 设计裁决与任务清单见 [plan/phase-7-browser-advisor.md](../plan/phase-7-browser-advisor.md)。
> **状态：v0（CLI）与 v1（仪表盘）已实现并离线测试；对手预留瞬间的 DOM 证据（E7 实验）
> 与真实房间人工验收尚未执行——预留识别当前只能覆盖"预留桌面牌"（由桌面差分确定），
> "预留牌堆"会诚实降级为灰色未知桶。**

### 9.1 它是什么

一个**零点击**的观察进程：你在它启动的 ego-browser 窗口里手动打牌，它每 0.4s（对手回合
0.25s）只读轮询页面，输出：

1. **走法建议**：GA 基因快评全部合法动作（★最优/☆次优 + 分数条 + 特征归因）；
   `--depth 2` 或仪表盘"深算"按钮触发 minimax 深算（含"对手最狠回应"行）；
2. **牌堆颜色直方图**：90 卡注册表 − 所有见过牌面 = 未翻开牌堆的层级×颜色分布
   （附灰色"未见去向"桶与守恒自检告警）；
3. **可负担性标尺**：每张可买牌"还差什么（金可补）"，桌面绿框 = 本回合可支付；
4. **预留记忆**：对手预留瞬间自动记录牌面、购买时核销（面板显示覆盖率与事件流）。

### 9.2 启动步骤

```bash
# ① 启动 advisor（会开一个 ego-browser 窗口——你就在这个窗口里创建/进入房间并打牌）
play-advisor --room-url https://game.hullqin.cn/ccbs/xxxx   # 房间号可省略，手动导航
#    仪表盘默认 http://localhost:8900 （--port 0 关闭；--depth 2 开 CLI 同步深算）

# ② 离线冒烟（无网络，验证安装）
play-advisor --fixture-html src/splendor/browser/fixtures/opening.html --max-frames 1 --port 0
```

### 9.3 纪律与已知局限

- **零点击**：advisor 包不含执行器（测试强制），对服务器只有被动只读 evaluate，
  无礼仪频率问题；但**打牌、选支付、选贵族、丢宝石全部由你手动完成**——
  面板在这些子流程会提示"建议暂停"。
- 引擎合法动作集 ⊆ 网页合法集：建议保证合法，但"少拿宝石"不在枚举（差异归因
  显示在面板奇偶区）；"预留牌堆顶"引擎本就不枚举（建议列表不会出现，直方图不受影响）。
- GA 评分在 2 人局内校准；3-4 人局建议只参考直方图/标尺，深算按钮仅 2 人局可用。
- 中途启动 advisor 时，此前已购牌面不可回溯，直方图灰桶会偏大（守恒自检兜底）。
