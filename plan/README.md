...
依赖图：`P0 → {P1 ∥ P2} → P3 → P4`；P5 贯穿。**关键路径先启动项：T0.4（网页实测）同时阻塞 P2 的两个任务，应最先安排。**
...
├────────────────────────────────────────────────────────────┤
│ L2 DQN 算法层（P1）：QNetwork │ ReplayBuffer │ 训练循环      │
│     —— 环境无关，只依赖 L1 协议 ——                           │
├────────────────────────────────────────────────────────────┤
...
核心原则：**单接口、双环境**。本地 `SplendorEnv` 与 `BrowserSplendorEnv` 实现同一协议，DQN checkpoint 不改一行代码即可两边运行；网页经验可回流本地 replay（off-policy 红利）。
