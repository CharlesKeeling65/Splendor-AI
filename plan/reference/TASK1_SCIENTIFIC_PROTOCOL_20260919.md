> 历史归档（2026-09-19）：本文件保留此前科学协议设计，不是当前执行指令。
> 当前路线见 [2p PPO 轻量提升计划](../task-1-2p-improvement-and-seed-protocol.md)。
> 旧 T1.4 统计退出门未被宣称完成；joint pilot、多 replicate、power/N 与 sealed 批次均已暂停。
> 不得依据本文恢复训练、扩展评测或消费受保护数据；需用户另行要求恢复正式研究路线。
...
- [x] 在任何新训练结果产生前实现并独立审计 `splendor-seed-roll/1`：对注册的
  `train-schedule` 有限总体作一次性 CSPRNG-root/HMAC 无放回排列，预留 5×32,000
  个互斥 replicate block，逐 replicate 精确平衡座次，并把 root、ScenarioV1 bank、
